...
import { parseMentions } from "$lib/postManipulation"
...
export const actions = {
	default: async ({ request, locals: { supabase } }) => {
        ...
        const urls: any = linkify.find(content)
        urls?.forEach((url: { value: string, type: string }) => {
            newContent = url.type !== "mention" ? newContent.replace(url.value, "") : newContent
		...
		redirect(301, `/app/post/${insertContent.data[0].id}`)
	},
} satisfies Actions
