...
export const load = (async ({ params, locals: { supabase } }) => {
    const getUser: any = await supabase.from("users").select("*").eq("slug", params.slug)
    ...
    if (!getUser.error) {
        return { reqUser: getUser.data[0], usersPost: retrievePosts()}
    } else {
        throw error(404)
    }
